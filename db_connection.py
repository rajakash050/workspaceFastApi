...
class DbConnection:
    ...
    def get_db(self, conn_string, db=""):
        conn_pool = getattr(self.threadLocal, 'conn_pool', {})
        con = conn_string + "_" + db
        try:
            if con in conn_pool and conn_pool[con] is not None:
                print("existing connection id is {}".format(id(conn_pool[con])))
                return conn_pool[con]
            else:
                rconn = self.reset_db_conn(conn_string, db)
                return rconn
    ...
    def reset_db_conn(self, conn_string, db=""):
        conn = None
        con_key = conn_string + "_" + db
        conn_pool = getattr(self.threadLocal, 'conn_pool', {})
        try:
        ...
        for i in range(3):
            try:
                conn = create_engine(conn_string + db)
                break
            except Exception as e:
                exc_obj = sys.exc_info()
                exc_type = exc_obj[0]
                ...
                continue
        if conn is not None:
            print("created...{0} for {1}".format(id(conn), db))
